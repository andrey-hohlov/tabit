...
var Tabit = function () {
  ...
  function Tabit(element) {
    ...
    // Already initialized
    var cachedInstance = Tabit._cacheGet(element);
    if (cachedInstance) {
  ...
  _createClass(Tabit, [{
    ...
    key: '_cacheSet',
    value: function _cacheSet(key, value) {
    ...
    key: '_cacheGet',
    value: function _cacheGet(key) {
      for (var i = 0, l = Tabit._cache.length; i < l; i += 1) {
        var cached = Tabit._cache[i];
        if (cached.key === key) {
          return cached.value;
        }
      }
    ...
    key: '_cacheRemove',
    value: function _cacheRemove(key) {
      for (var i = 0, l = Tabit._cache.length; i < l; i += 1) {
        var cached = Tabit._cache[i];
        if (cached.key === key) {
